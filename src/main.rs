...
fn main() -> io::Result<()> {
    ...
    loop {
            ...
            if verb.eq_ignore_ascii_case("take") || verb.eq_ignore_ascii_case("get") {
                if rest.is_empty() {
                    ...
                    engine::handle_take_all_room(
                        &mut out, &mut item_locations, &world, &current_room_id, &flags
                    );
                    ...
                    if item_part.is_empty() || container_part.is_empty() {
                        ...
                        engine::handle_take_all_from_container(
                            &mut out, &mut item_locations, &world, &current_room_id, container_part, &flags
                        );
                    } else {
                        engine::handle_take_from_container(
                            &mut out, &mut item_locations, &world, &current_room_id, item_part, container_part, &flags
                        );
                    }
                } else {
                    engine::handle_take(
                        &mut out, &mut item_locations, &world, &current_room_id, &rest_lower, &flags
                    );
                }
                ...
                || (verb.eq_ignore_ascii_case("look") && rest_lower.starts_with("at "))
            {
                ...
                if target.is_empty() {
                    ...
                    engine::handle_examine(
                        &mut out, &world, &item_locations, &current_room_id, target, &flags
                    );
                }
                ...
                } else if engine::try_handle_movement(
                    &mut out, &mut current_room_id, &world, current_room, &lower, &flags
                ) {
                    if let Some(room) = world.rooms.get(&current_room_id) {
                        engine::render_room(&mut out, room, &flags, &world, &item_locations);
                        rendered_room_this_turn = true;
        ...
        if !changed_flags.is_empty() && !rendered_room_this_turn {
            if let Some(room) = world.rooms.get(&current_room_id) {
                if engine::room_depends_on_any_flag(room, &world, &item_locations, &changed_flags) {
                    engine::render_room(&mut out, room, &flags, &world, &item_locations);
                }
            }
        }
